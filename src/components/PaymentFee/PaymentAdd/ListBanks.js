...
import ListMcc from './modal/ListMcc';
import UpdateMcc from './modal/UpdateMcc';
...
class ListBanks extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      idMerchant: null,
      merchantCode: '',
      isShowModalListMcc: false,
      isShowModalUpdateMcc: false,
      listMccDetail: [],
    };
  }
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if(history.location.pathname !== '/merchant/add-fee-merchant'){
      return {
        tDataTable: listMccNational.length > 0 ? listMccNational : [],
        totalRow:  listMccNational.length > 0 ? listMccNational.length : 0,
        dataInfoGeneral,
        dataInfoMerchantApply,
        availableData,
      };
  ...
  openToggleListMcc(dataMcc) {
    this.setState({
      listMccDetail: dataMcc,
    })
    this.toggleListMcc();
  }

  toggleListMcc = () => {
    const { isShowModalListMcc } = this.state;
    this.setState({
      isShowModalListMcc: !isShowModalListMcc,
    });
  }

  openToggleUpdateMcc(dataMcc) {
    // this.setState({
    //   listMccDetail: dataMcc,
    // })
    this.toggleUpdateMcc();
  }

  toggleUpdateMcc = () => {
    const { isShowModalUpdateMcc } = this.state;
    this.setState({
      isShowModalUpdateMcc: !isShowModalUpdateMcc,
    });
  }
      ...
      <>
        <Col md={12}>
                  ...
                  ? (tDataTable
                    && tDataTable.slice(currentPage * perPage - perPage, perPage * currentPage)
                      .map((data, index) => (
                        <tr key={index}>
                          <td>{ index + 1 }</td>
                          <td>{data.payChannelSet.label}</td>
                          <td>{data.typePayment.length}</td>
                          <td>{data.feeCodeAndFeeName.feeName}</td>
                          <td>{data.mccNational.length}</td>
                          <td className="icon-fee text-left">
                            <div>
                              <Button
                                ...
                                data-placement="top"
                                title="Chi tiết"
                                onClick={() => this.openToggleListMcc(data)}
                              >
                                <i className="icon-info" />
                              </Button>
                              <Button
                                title="Cập nhật"
                                onClick={() => this.openToggleUpdateMcc(data)}
                              >
                                <i className="icon-note" />
                              </Button>
                              <Button className="open-lock" title="Delete" onClick={() => this.openDeleteFeeBanks({data, index})}>
          ...
          <div>
            ...
            />
            <ListMcc
              toggleListMcc={this.toggleListMcc}
              isShowModalListMcc={isShowModalListMcc}
              listMccDetail={listMccDetail}
            />
            <UpdateMcc
              toggleUpdateMcc={this.toggleUpdateMcc}
              isShowModalUpdateMcc={isShowModalUpdateMcc}
              // listMccDetail={listMccDetail}
            />
...
const mapStateToProps = (state, ownProps) => ({
  ...
  listMccNational: state.merchantCommon.listMccNational,
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(ListBanks));
